...
<head>
    ...
    <title>メインページ | 陶芸体験ワークショップ〔統合版〕</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/css/style.css">
</head>
<body>
<header class="site-header">
    <div class="site-header-inner">
        <h1 class="site-title">陶芸体験ワークショップ〔統合版〕</h1>
        <nav class="site-nav">
            <span class="welcome">${sessionScope.cbLoginMember.name} さん</span>
            <a href="${pageContext.request.contextPath}/combined/user/reserve?action=mylist">マイ予約</a>
            <a href="${pageContext.request.contextPath}/combined/logout" class="btn btn-sm btn-outline">ログアウト</a>
        </nav>
    </div>
</header>
        ...
        <div class="calendar-header">
            ...
            <div class="calendar-nav">
                <a class="btn btn-sm btn-outline"
                   href="${pageContext.request.contextPath}/combined/user/main?year=${prevYear}&month=${prevMonth}">‹ 前月</a>
                <span class="calendar-current">${calYear}年 ${calMonth}月</span>
                <a class="btn btn-sm btn-outline"
                   href="${pageContext.request.contextPath}/combined/user/main?year=${nextYear}&month=${nextMonth}">翌月 ›</a>
            </div>
        </div>
                            ...
                            <c:forEach var="w" items="${workshops}">
                                ...
                                <c:if test="${wDate == target}">
                                    <a class="cal-event ${w.full ? 'full' : ''}"
                                       href="${pageContext.request.contextPath}/combined/user/reserve?id=${w.workshopId}"
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a class="btn btn-primary btn-block"
                                       href="${pageContext.request.contextPath}/combined/user/reserve?id=${w.workshopId}">予約する</a>
                                </c:otherwise>
                            </c:choose>
                        </article>
                    </c:forEach>
