...
| テーブル | 概要 | 主なカラム |
|----------|------|-----------|
| `members`      | 会員（一般・管理者） | name, email(UNIQUE), phone, address, password(ハッシュ), role |
| `workshops`    | ワークショップ      | title, description, instructor, event_date, start_time, capacity, price |
| `reservations` | 予約               | member_id(FK), workshop_id(FK), number_of_people, status |
...
## 補足
- パスワードは学習用途のため **SHA-256** でハッシュ化（実運用では bcrypt 等を推奨）。
